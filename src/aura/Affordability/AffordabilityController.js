        ...
        action.setCallback(this, function(response) {
            ...
            if($A.util.isUndefinedOrNull(tabledata) != true) {
                ...
                for(var i = 0 ; i < data.simulationData.profileData.financialHealthAnalysis.length ; i++){
                    d1.push([(offset+i), tabledata.simulationData.profileData.financialHealthAnalysis[i].score]);
                    chartLabels.push(offset+i);
                    chartDataSet.push(tabledata.simulationData.profileData.financialHealthAnalysis[i].score.toPrecision(2));
                }
                
                var greenGoalMsg=false;
                var greenGoalMsgCounter=-1;
                var diagnosis = tabledata;
